...
> 모델 표면 정의 형식
* 넙스/쿼드/삼각형 등
* 파이프라인 런타임 단계에서 렌더링하려면 반드시 삼각형으로 테셀레이션해야 한다.

> 스키닝
* 정점 중에는 스키닝 할 수 있는 것이 있다.
* 정점을 하나 이상의 뼈대 구조 관절들에 연결 시키고 관절들이 해당 정점에 미치는 영향 가중치도 지정한다.
* 애니메이션 시스템에서 모델의 움직임을 표현할 때 사용 된다.

> material
* material에 쓰일 Shader를 고르고 Shader에 필요한 Texture를 선택하고 각 Shader의 설정 인자와 옵션들을 지정한다.
* material은 메시와 같이 저장하기도 하지만 이렇게 되면 데이터가 중복 되니 대부분 게임에서는 소수의 material을 만들고 여러 물체에 재사용한다.

### 10.2.3 자원 다듬기 단계

해당 단계에서는 여러 종류의 자원들을 뽑아내어 가공해서 일관된 단위로 만든다.
...
위의 3D 모델이 필요로하는 자원들을 엔진에서 바로 불러와 사용할 수 있게 하는 것이 이 단계의 역할이다

> 기하형상/material 저장
* 플랫폼 중립적인 중간형식으로 저장한다.
* 이 데이터를 가공해서 각가 플랫폼에 특환된 형식으로 변환한다.
* 이렇게 변화된 플렛폼 특화 자원들은 별도 처리하지 않고도 런타임에 메모리에 올릴 수 있다.
...
> 파티클 특징
* 항상 카메라를 향하고 있도록 한다.
  * 파티클은 대부분 "쿼드"로 만들어지기 때문이다.
* 렌더링 순서가 중요하다.
  * 파티클은 대부분 불투명하기 때문이다.
* 매 프레임마다 파티클 위치/방향/크기/텍스처 좌표/셰이더 전달 인자등이 달라진다.
* 파티클은 정해진 생성 속도에 따라 생성되고 삭제 된다.(파티클 Emitler)

### 10.4.2 데칼

> Decal
* 기하 형상들의 표면에 씌우는 작은 기하 형상.
* ex) 총탄 흔적, 발자국, 긁힌 흔적 등

> 구현 방법
* 데칼을 사각형 영역으로 모델링한다.
* 이 모델링을 일직선으로 투영한다.
* 이렇게 하면 3차원 공간에 직육면체 프리즘이 생긴다.
* 이 프리즘이 가장 먼저 교차하는 표면에 데칼을 입힌다.
* 교차하는 표면의 삼각형들을 "프리즘"으로 클리핑한다.
* 각 정점마다 텍스처 좌표를 계산하고 매핑한다.
* 이 때 깊이감을 주기 위해 parallax mapping(질문7)을 하기도 한다.
...
> 단순한 방법
* 기하 현상을 렌더링하기 전에 프레임버퍼에 하늘 텍스처를 채워 놓는다.

> 플레이어가 자유롭게 모든 방향을 바라보는 경우
* 돔이나 박스 형태로 그린다.
* 카메라 중심으로 그리면 항상 멀리 있는 것처럼 보이게 만들 수 있다.
...
* ??
> 5. scene graph
* https://bobbybong.tistory.com/m/entry/Scene-Graph의-원리
